...
type ViewType = "summary" | "active" | "failed" | "dead" | "error-rate" | "latency";
...
export default function OpsPage() {
    ...
    const handleReplay = async (jobId: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/jobs/${jobId}/replay`, { method: "POST" });
                ...
                return (
                    <div className="space-y-6">
                            ...
                            <CardContent className="h-[300px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData}>
                                        ...
                                        />
                                        <Area type="monotone" dataKey="success" stroke="#3b82f6" fillOpacity={1} fill="url(#colorSuccess)" name="Completed" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                        <Card>
                            ...
                            <CardContent>
                                <JobTable jobs={data?.activeJobs || []} onReplay={handleReplay} />
                            </CardContent>
                        </Card>
                    </div>
                );
                ...
                return (
                    <div className="space-y-6">
                            ...
                            <CardContent className="h-[300px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={chartData}>
                                        ...
                                        <YAxis fontSize={12} tickLine={false} axisLine={false} />
                                        <Tooltip />
                                        <Line type="monotone" dataKey="failed" stroke="#ef4444" strokeWidth={2} name="Failures" dot={{ r: 4 }} activeDot={{ r: 6 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                        <Card>
                            ...
                            <CardContent>
                                <JobTable jobs={data?.failedJobs || []} onReplay={handleReplay} />
                            </CardContent>
                        </Card>
                    </div>
                );
                ...
                return (
                    <div className="space-y-6">
                        <Card>
                            ...
                            <CardContent>
                                <JobTable
                                    jobs={data?.failedJobs.filter(j => j.status === 'dead') || []}
                ...
                return (
                    <div className="space-y-6">
                            ...
                            <CardContent className="h-[300px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData}>
                                        ...
                                        <Tooltip />
                                        <Area type="monotone" dataKey="avg_duration" stroke="#10b981" fill="#10b981" fillOpacity={0.1} name="Avg Latency" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                        <Card>
                            ...
                            <CardContent>
                                <JobTable jobs={data?.recentJobs.filter(j => j.status === 'completed') || []} onReplay={handleReplay} />
                            </CardContent>
                        </Card>
                    </div>
                );
                ...
                return (
                    <div className="space-y-6">
                            ...
                            <CardContent className="h-[300px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={chartData}>
                                        ...
                                        <YAxis fontSize={12} tickLine={false} axisLine={false} />
                                        <Tooltip />
                                        <Line type="monotone" dataKey="success" stroke="#3b82f6" strokeWidth={2} name="Success" />
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <JobTable jobs={data?.recentJobs || []} onReplay={handleReplay} />
                            </CardContent>
                        </Card>
                    </div>
                );
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">
                            {selectedView === "summary" ? "Operations Dashboard" : `${statsConfig.find(s => s.id === selectedView)?.title} Details`}
                        </h1>
                        <p className="text-sm text-muted-foreground">
                            Auto-updates every 10s • Last updated: {lastUpdated?.toLocaleTimeString() || "..."}
                        </p>
                    </div>
                </div>
                ...
                {statsConfig.map((stat) => (
                    ...
                    >
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            ...
                            <stat.icon className={`h-4 w-4 ${stat.color} ${stat.variant === "destructive" ? "animate-pulse" : ""}`} />
                        </CardHeader>
                        <CardContent>
...
function JobTable({ jobs, onReplay }: { jobs: any[], onReplay: (jobId: string) => void }) {
                    ...
                    return (
                        <>
                            ...
                            >
                                ...
                                <TableCell className="font-mono text-xs font-medium">{job.type}</TableCell>
                                <TableCell>
                                            ...
                                            variant={
                                                job.status === "completed" ? "default" :
                                                    job.status === "failed" ? "destructive" :
                                                        job.status === "dead" ? "destructive" :
                                                            job.status === "running" ? "outline" :
                                                                "secondary"
                                            }
                                            className={
                                                job.status === "running" ? "animate-pulse border-blue-200 bg-blue-50 text-blue-700" :
                                                    job.status === "dead" ? "bg-gray-900 text-white hover:bg-gray-800" : ""
                                            }
                                        >
                                ...
                                <TableCell className="text-sm">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-xs text-muted-foreground">{job.progress} / {job.total}</span>
                                ...
                                <TableCell className="text-right">
                                    <div className="flex flex-col items-end gap-1">
                                        {job.durationMs && <span className="text-xs font-mono">{job.durationMs}ms</span>}
                                        {job.error && (
                                            <span className="text-[10px] text-red-500 max-w-[150px] truncate" title={job.error}>
                                                {job.error}
                                            </span>
                                        )}
                                        {job.deadLetteredAt && <span className="text-[10px] text-gray-500">DLQ: {new Date(job.deadLetteredAt).toLocaleTimeString()}</span>}
                                    </div>
                                </TableCell>
                                <TableCell className="text-right">
                                    ...
                                    <TableCell colSpan={7}>
                                        <div className="p-4 space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1">
                                                    <h4 className="text-xs font-semibold text-muted-foreground">Payload</h4>
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <pre className="text-[10px] font-mono bg-background p-2 rounded border overflow-auto max-h-[200px]">
                                                        {job.result ? JSON.stringify(job.result, null, 2) : <span className="text-muted-foreground italic">No result data</span>}
                                                    </pre>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-4 gap-4 text-xs text-muted-foreground border-t pt-4">
                                                ...
                                                <div>
                                                    ...
                                                    <span>{job.retryCount} / {job.maxRetries || 5}</span>
                                                </div>
                                                <div>
                                                ...
                                                <div>
                                                    <span className="font-semibold block">Finished At</span>
                                                    <span>{job.finishedAt ? new Date(job.finishedAt).toLocaleString() : '-'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </TableCell>
