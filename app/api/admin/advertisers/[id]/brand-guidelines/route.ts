...
import { attachAdvertiserBrandGuidelines, detachAdvertiserBrandGuidelines, getAdvertiserBrandGuidelines } from "@/features/admin/services/brandGuidelines.service";
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (body.type === "file") {
            return NextResponse.json({ error: "File uploads are not yet supported. Please use URL or text type." }, { status: 400 });
        }
        ...
        await attachAdvertiserBrandGuidelines(id, brandGuidelines, session.user.id);
