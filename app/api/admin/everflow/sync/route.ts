...
export async function POST(req: Request) {
    ...
    try {
        ...
        logger.everflow.info(`Everflow sync job created - jobId: ${job.id}, route: /api/admin/everflow/sync, userId: ${session.user.id}`);
        ...
        logger.everflow.error(`Failed to start sync job - route: /api/admin/everflow/sync, error: ${message}`);
