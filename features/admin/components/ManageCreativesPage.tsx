/**
 * ManageCreativesPage - Admin's view of approved and rejected creatives
 *
 * This page displays all creative requests that have been approved or rejected,
 * regardless of approval stage. It shows the final state of creatives.
 *
 * TABS BREAKDOWN:
 * - Approved: All creatives with status='approved' (completed workflow)
 * - Rejected: All creatives with status='rejected' (rejected at any stage)
 */
...
export function ManageCreativesPage() {
  const variables = getVariables();
  const { requests, isLoading, error } = useManageCreativesViewModel();
  const [activeTab, setActiveTab] = useState<TabValue>("approved");
