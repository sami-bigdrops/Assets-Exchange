...
export async function listOffers({ search, status }: { search?: string; status?: "Active" | "Inactive" }) {
    const conditions = [];
...
export async function createOffer(data: {
    ...
    const [advertiser] = await db.select().from(advertisers).where(eq(advertisers.id, data.advertiserId));
    if (!advertiser) {
...
export async function updateOffer(id: string, data: Partial<{
    ...
    const [row] = await db.update(offers).set({
        offerName: data.name,
    ...
    for (const id of offerIds) {
        try {
            ...
            if (row) {
                ...
                results.failed.push({ offerId: id, error: "Not found", reason: "Offer does not exist" })
            }
        } catch (e: unknown) {
            results.failed.push({ offerId: id, error: "Update failed", reason: e instanceof Error ? e.message : String(e) })
        }
    }
