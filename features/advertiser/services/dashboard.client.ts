...
export async function getAdvertiserDashboardData(): Promise<AdminDashboardData> {
  try {
    const response = await fetch("/api/advertiser/dashboard/stats", {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new Error("Unauthorized: Please log in as advertiser");
      }
      throw new Error(
    ...
    console.error("Error fetching advertiser dashboard stats:", error);
    throw error;
  }
}
